    ...
    <div className="min-h-screen">
      ...
      <header className="border-b">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <Link href="/auth/login">
            <Button>Начать через Telegram</Button>
          </Link>
        </div>
      </header>
      ...
      <section className="container mx-auto px-4 py-16 md:py-24 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          ...
          <div className="flex gap-2 text-sm text-muted-foreground">
            <Link href="/auth/login?method=max" className="hover:underline">MAX</Link>
            <span>·</span>
            <Link href="/auth/login?method=email" className="hover:underline">Email</Link>
          </div>
        </div>
      </section>
          ...
          {[
            {
              step: 1,
              title: "Подключитесь",
              description: "через Telegram, MAX или Email"
            },
            {
              step: 2,
      ...
      <section id="try-free" className="bg-muted/50 py-16">
        <div className="container mx-auto px-4 text-center">
          ...
          <Link href="/auth/login">
            <Button size="lg">Начать через Telegram</Button>
          </Link>
          <div className="flex gap-2 justify-center mt-4 text-sm text-muted-foreground">
            <Link href="/auth/login?method=max" className="hover:underline">Войти через MAX</Link>
            <span>·</span>
            <Link href="/auth/login?method=email" className="hover:underline">Войти по Email</Link>
          </div>
        </div>
      </section>
